...
$sql=mysqli_query($conn, "SELECT * FROM tb_barang_rusak WHERE id='$_GET[kode]'");
...
if(isset($_POST['proses'])){
  ...
  mysqli_query($conn, "UPDATE tb_barang_rusak SET
  nama_barang = '$_POST[barang]',
  serial_number = '$_POST[serial]',
  mac = '$_POST[mac]'
  WHERE id = '$_GET[kode]'");
}
echo "<meta http-equiv=refresh content=30;URL='barang-rusak.php'>";
...
}elseif(isset($_POST['status_idle'])){

    $barang = ($_POST['barang']);
    $serial = ($_POST['serial']);
    $mac = ($_POST['mac']);
    $status = ($_POST['status_idle']);
  mysqli_query($conn, "INSERT INTO tb_barang_idle(nama_barang, serial_number, mac, status_barang_idle)
  VALUES ('$barang', '$serial', '$mac', '$status')");
}
